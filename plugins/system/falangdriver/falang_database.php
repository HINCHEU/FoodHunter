...
class JFalangDatabase extends JOverrideDatabase {
	...
	public function __construct($options)
    {
            ...
            if( !isset($this->_mlTableList) ){
                Factory::getApplication()->enqueueMessage(TEXT::_('No valid table list:'),'error');
            }
     ...
     *
     *  @since 4.5 change onBeforeTranslation parameters order
	...
	function getNumRows( $cur=null, $translate=true, $language=null )
	{
        ...
        // Get the dispatcher and load the falang plugins.
		...
		if ($defaultLang == $language){
			$rows = array($count);
            Factory::getApplication()->triggerEvent('onBeforeTranslation', array (&$rows, &$ids, $reference_table, $language, $this->getRefTables(), $this->sql, $refTablePrimaryKey, $allowfallback));
			$count = $rows[0];
			return $count;
		}
        ...
        Factory::getApplication()->triggerEvent('onAfterTranslation', array (&$rows, &$ids, $reference_table, $language, $this->getRefTables(), $this->sql, $refTablePrimaryKey, $allowfallback));
		$count = $rows[0];
		return $count;
	}
...
//				if ($jfManager->getCfg("transcaching",1)){
	...
	*/
    ...
    function updateObject( $table, &$object, $keyName, $updateNulls=true, $passthru=false ) {
        ...
        if (($table=='#__content' || $table=='#__modules' ) && isset($object->id) &&$object->id>0 ){
            if ($table=='#__content'){$test = JTable::getInstance("Content");}
            if ($table=='#__modules'){$test = JTable::getInstance("Module");}
            $test->load($object->id);
            if ( $test->language == '*'){
