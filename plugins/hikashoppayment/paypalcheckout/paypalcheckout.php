...
class plgHikashoppaymentPaypalcheckout extends hikashopPaymentPlugin
{
	...
	private function getOrderData(&$order) {
		...
		$orderData->payment_source = new stdClass();
		$orderData->payment_source->paypal = new stdClass();
		$orderData->payment_source->experience_context = new stdClass();
		if(!empty($this->payment_params->brand_name))
			$orderData->payment_source->experience_context->brand_name = mb_substr($this->payment_params->brand_name, 0, 127);
		$orderData->payment_source->experience_context->cancel_url = HIKASHOP_LIVE.'index.php?option=com_hikashop&ctrl=order&task=cancel_order&order_id='.$order->order_id . $this->url_itemid;
		if(!empty($this->payment_params->landing_page)) {
			$orderData->payment_source->experience_context->landing_page = $this->payment_params->landing_page;
		}
		$orderData->payment_source->experience_context->user_action = 'PAY_NOW';
