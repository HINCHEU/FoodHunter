...
$input = '<input type="text" name="q" id="mod-finder-searchword' . $module->id . '" class="js-finder-search-query jl-search-input" value="' . htmlspecialchars($app->input->get('q', '', 'string'), ENT_COMPAT, 'UTF-8') . '"'
...
if ($params->get('show_button', 0)) {
    $output .= $label;
	$output .= '<div class="jl-margin" jl-margin>';
    $output .= $input;
    $output .= '<button class="jl-button jl-button-primary" type="submit"><span class="icon-search icon-white" aria-hidden="true"></span> ' . Text::_('JSEARCH_FILTER_SUBMIT') . '</button>';
    $output .= '</div>';
} else {
    $output .= $label;
    $output .= $input;
...
<form class="mod-finder js-finder-searchform jl-search jl-search-default" action="<?php echo Route::_($route); ?>" method="get" role="search">
    <?php echo $output; ?>
